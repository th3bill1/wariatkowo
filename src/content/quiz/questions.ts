...
export const quizQuestions: QuizQuestion[] = [
  ...
  },
  {
    ...
    answers: [
      ...
      "bo Misiek krzyczy na komputer",
      "Wcale nie jest głośno",
      "bo słychać głośne jęki",
    ],
    correctAnswer: 3,
  },
];
...
export function validateQuizQuestions(
  ...
  questions.forEach((question, index) => {
    ...
    if (!question.id.trim()) {
      ...
      issues.push({
        id: question.id,
        message: `Pytanie ${index + 1}: duplikat id "${question.id}".`,
      });
    } else {
      ids.add(question.id);
    }
    ...
    if (!question.question.trim()) {
      issues.push({
        id: label,
        message: `Pytanie ${index + 1}: treść pytania nie może być pusta.`,
      });
    }
    ...
    if (question.answers.length !== 4) {
      issues.push({
        id: label,
        message: `Pytanie ${index + 1}: musi mieć dokładnie 4 odpowiedzi.`,
      });
    }
    ...
    if (
      ...
      issues.push({
        id: label,
        message: `Pytanie ${index + 1}: correctAnswer musi być liczbą od 0 do 3.`,
      });
    }
    ...
    if (question.answers.some((answer) => !answer.trim())) {
      issues.push({
        id: label,
        message: `Pytanie ${index + 1}: wszystkie odpowiedzi powinny być niepuste.`,
      });
    }
  });
