  ...
  switch (action.type) {
    ...
    case "choose-answer": {
      if (
        ...
        state.evaluation.status !== "none"
      ) {
        return state;
      }

      const correctAnswerIndex =
        quizQuestions[state.currentQuestionIndex].correctAnswer;
...
export function ReturnToWariatkowoPage() {
  ...
  const currentQuestion =
    quizQuestions[state.currentQuestionIndex] ?? quizQuestions[0];
  ...
  const resultMessage = useMemo(
    () => getQuizResultMessage(percentageScore),
    [percentageScore],
  );
    ...
    return (
      <div className="quiz-page">
        <PageHeader
          ...
          actions={
            <Link className="app-link-button" to="/dashboard">
              ← Wróć do Wariatkowa
            </Link>
          }
        />
        <AppCard className="quiz-error-card">
          ...
          <ul className="quiz-error-card__list">
            {quizQuestionIssues.map((issue, index) => (
              <li key={`${issue.id ?? "quiz-issue"}-${index}`}>
                {issue.message}
              </li>
            ))}
          </ul>
  ...
  return (
    <div className="quiz-page">
          ...
          <>
            ...
            <QuestionCard
              ...
              onChooseAnswer={(answerIndex) =>
                dispatch({ type: "choose-answer", answerIndex })
              }
              onChooseMultipleChoice={() =>
                dispatch({ type: "choose-multiple-choice" })
              }
              onChooseSelfAnswer={() =>
                dispatch({ type: "choose-self-answer" })
              }
              onEvaluateSelfAnswer={(isCorrect) =>
                dispatch({ type: "evaluate-self-answer", isCorrect })
              }
      ...
      <p className="quiz-page__footnote">
        Pamiętaj: po kliknięciu <strong>Pokaż odpowiedzi</strong> to pytanie
        daje maksymalnie 1 pkt.
      </p>
    </div>
  );
}
