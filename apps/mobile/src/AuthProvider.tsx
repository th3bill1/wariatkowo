...
const baseUrl = (process.env.EXPO_PUBLIC_API_URL || Constants.expoConfig?.extra?.apiUrl || "").replace(/\/$/, "");
...
export function AuthProvider({ children }: PropsWithChildren) {
  ...
  const api = useMemo(() => createApiClient({ baseUrl, tokenStore: sessionStore, onUnauthorized: () => { void sessionStore.clear(); setMember(null); } }), []);
  ...
  const login = useCallback(async () => {
    ...
    const result = await WebBrowser.openAuthSessionAsync(`${baseUrl}/api/auth/mobile?redirect_uri=${encodeURIComponent(redirect)}`, redirect);
    if (result.type !== "success") return;
    const code = new URL(result.url).searchParams.get("code");
    if (!code) throw new Error("Logowanie nie zwróciło kodu sesji.");
    const exchanged = await api.auth.exchange(code); await sessionStore.set(exchanged.token); await configureWidget(baseUrl, exchanged.token); setMember(exchanged.member);
  }, [api]);
