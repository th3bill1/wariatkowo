...
export function AuthProvider({ children }: PropsWithChildren) {
  ...
  const restore = useCallback(async () => {
    try {
      const restoredMember = await api.auth.session();
      setMember(restoredMember);
      const token = await sessionStore.get();
      if (restoredMember && token) await configureWidget(apiBaseUrl, token);
    } catch {
      setMember(null);
    } finally {
      setLoading(false);
  ...
  const login = useCallback(async () => {
    ...
    const result = await WebBrowser.openAuthSessionAsync(
      `${apiBaseUrl}/api/auth/mobile?redirect_uri=${encodeURIComponent(redirect)}`,
      redirect,
    );
    if (result.type !== "success") return;
    const code = new URL(result.url).searchParams.get("code");
    if (!code) throw new Error("Logowanie nie zwróciło kodu sesji.");
    const exchanged = await api.auth.exchange(code);
    await sessionStore.set(exchanged.token);
    await configureWidget(apiBaseUrl, exchanged.token);
    setMember(exchanged.member);
  }, [api]);
  ...
  return (
    <Context.Provider value={{ member, loading, api, login, logout }}>
      {children}
    </Context.Provider>
  );
}
