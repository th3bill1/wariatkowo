...
export async function onRequest(context:{request:Request;env:Env}):Promise<Response>{
 ...
 if(!Number.isInteger(requested)||requested<1||requested>90) return error('VALIDATION_ERROR','Zakres statystyk musi wynosić od 1 do 90 dni.');
 ...
 ).bind(since).all<{id:string;name:string;slug:HouseholdMemberSlug;count:number}>();
 ...
 ).bind(since).all<{id:string;title_snapshot:string;completed_at:string;member_id:string;name:string;slug:HouseholdMemberSlug}>();
 const data:TaskStats={
  days:requested,
  members:members.results.map(row=>({id:row.id,name:row.name,slug:row.slug,count:Number(row.count)})),
  sharedCount:Number(shared?.count??0),
  recentActivity:activity.results.map(row=>({
   id:row.id,title:row.title_snapshot,completedAt:row.completed_at,
