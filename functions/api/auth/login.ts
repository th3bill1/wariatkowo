import {
  createSessionCookie, createSessionExpiry, isValidPin, loadMemberForLogin,
  sha256, verifyMemberPin,
} from '../../_shared/auth';
import { error, methodNotAllowed, nowIso, readJsonBody, success, type Env } from '../../_shared/http';

const MAX_FAILURES = 5;
const WINDOW_MINUTES = 15;

function clientIdentity(request: Request): string {
  return (request.headers.get('CF-Connecting-IP') ?? 'unknown') + '|' + (request.headers.get('User-Agent') ?? 'unknown');
}
export async function onRequest(context: { request: Request; env: Env }): Promise<Response> {
  if (context.request.method !== 'POST') return methodNotAllowed(['POST']);
  let body: unknown;
  try { body = await readJsonBody(context.request); } catch {
    return error('VALIDATION_ERROR', 'Nie udało się zalogować. Sprawdź profil i PIN.');
  }
  const input = body as { memberId?: unknown; pin?: unknown };
  if (typeof input.memberId !== 'string' || !isValidPin(input.pin)) {
    return error('UNAUTHORIZED', 'Nie udało się zalogować. Sprawdź profil i PIN.', 401);
  }
  ...
  ).bind(await sha256(input.memberId), clientHash, since).first<{ count: number }>();
  if ((failures?.count ?? 0) >= MAX_FAILURES) {
    return error('RATE_LIMITED', 'Za dużo prób. Odczekaj chwilę i spróbuj ponownie.', 429);
  }
  ...
  ).bind(await sha256(input.memberId), clientHash, timestamp, valid ? 1 : 0).run();
  ...
  await context.env.DB.batch([
    ...
    ).bind(crypto.randomUUID(), await sha256(token), member.id, expiresAt.toISOString(), timestamp, timestamp),
    context.env.DB.prepare('DELETE FROM login_attempts WHERE attempted_at < ?')
