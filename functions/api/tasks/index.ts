...
import { isTaskAssignment, parseRecurrence, TASK_COLUMNS, toTask, type TaskRow } from '../../_shared/tasks';
...
async function getTasks(env: Env): Promise<Task[]> {
  const result = await env.DB.prepare(
    'SELECT ' + TASK_COLUMNS + ' FROM tasks ORDER BY is_completed ASC, CASE WHEN due_date IS NULL THEN 1 ELSE 0 END ASC, due_date ASC, sort_order ASC, created_at ASC',
  ).all<TaskRow>();
...
async function createTask(env: Env, body: unknown): Promise<Response> {
  ...
  ).bind(id,title,notes??null,dueDate??null,(maxSort?.value??-1)+1,assignment,recurrence?.unit??null,recurrence?.interval??null,seriesId,timestamp,timestamp).run();
  const created=await env.DB.prepare('SELECT '+TASK_COLUMNS+' FROM tasks WHERE id = ?').bind(id).first<TaskRow>();
  return created ? success(toTask(created),{status:201}) : error('INTERNAL_ERROR','Nie udało się utworzyć zadania.',500);
}
export async function onRequest(context:{request:Request;env:Env}):Promise<Response>{
  const auth=await requireAuth(context.request,context.env); if(isAuthResponse(auth)) return auth;
  if(context.request.method==='GET') return success(await getTasks(context.env));
  if(context.request.method==='POST'){
    try{return await createTask(context.env,await readJsonBody(context.request));}
    catch{return error('VALIDATION_ERROR','Treść żądania nie jest poprawnym JSON-em.');}
  }
