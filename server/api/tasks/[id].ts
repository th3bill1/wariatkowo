...
async function loadTask(env: Env, id: string): Promise<TaskRow | null> {
  return env.DB.prepare("SELECT " + TASK_COLUMNS + " FROM tasks WHERE id = ?")
    .bind(id)
    .first<TaskRow>();
}
async function updateTask(
  ...
  const title =
    input.title === undefined ? current.title : parseTrimmedString(input.title);
  const notes =
    input.notes === undefined
      ? current.notes
      : parseOptionalString(input.notes);
  const dueDate =
  ...
  const assignment =
    input.assignment === undefined ? current.assignment : input.assignment;
  const recurrence: TaskRecurrence | null | undefined =
    input.recurrence === undefined
      ? current.recurrence_unit && current.recurrence_interval
        ? {
            unit: current.recurrence_unit,
  ...
  if (input.sortOrder !== undefined && sortOrder === undefined)
  ...
  const becomingComplete = isCompletionTransition(
    current.is_completed === 1,
    completed,
  );
  const becomingIncomplete = isUncompletionTransition(
    current.is_completed === 1,
    completed,
  );
  ...
  const seriesId = recurrence
    ? (current.recurrence_series_id ?? crypto.randomUUID())
    : null;
  const update = contextStatement(
    env,
  ...
  if (becomingIncomplete) {
    statements.push(
      contextStatement(
        env,
        "DELETE FROM task_completion_events WHERE task_id = ?",
        [id],
      ),
      contextStatement(
        env,
        "DELETE FROM tasks WHERE generated_from_task_id = ? AND is_completed = 0",
        [id],
      ),
    );
  }
  ...
  return updated
    ? success(toTask(updated))
    : error("INTERNAL_ERROR", "Nie udało się zaktualizować zadania.", 500);
}
function contextStatement(
  env: Env,
  sql: string,
  values: unknown[],
): DatabaseStatement {
...
export async function onRequest(context: {
  ...
  if (context.request.method === "PATCH") {
    try {
      ...
      return error("VALIDATION_ERROR", "Nie udało się zaktualizować zadania.");
    }
  }
  if (context.request.method === "DELETE") {
    if (!(await loadTask(context.env, id)))
