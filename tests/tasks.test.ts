...
describe("task rules", () => {
  it("validates assignment modes", () => {
    for (const value of ["anyone", "misiek", "miska", "both"])
      expect(isTaskAssignment(value)).toBe(true);
    expect(isTaskAssignment("someone")).toBe(false);
  });
  it("validates recurrence ranges", () => {
    expect(parseRecurrence({ unit: "day", interval: 5 })).toEqual({
      unit: "day",
      interval: 5,
    });
  ...
  it("calculates daily, weekly, and month-end recurrence", () => {
    expect(
      calculateNextDueDate("2026-08-08T00:00:00.000Z", {
        unit: "day",
        interval: 5,
      }),
    ...
    expect(
      calculateNextDueDate("2026-01-31T00:00:00.000Z", {
        unit: "month",
        interval: 1,
      }),
