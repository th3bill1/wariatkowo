...
Expo prebuild installs a config-plugin-managed Jetpack Glance widget. It displays exactly Boskie światło, Miśkolampa, and Szumownica, uses the same Lucide bulb/snowflake semantics as the web cards, performs authenticated API toggles in an Android callback, and opens `wariatkowo://devices/{boskie-swiatlo|miskolampa|szumownica}` for full controls. Widget credentials are copied into encrypted Android preferences, restored from secure storage after an app restart, cleared on logout, and never include Home Assistant credentials. Android requests a state snapshot before toggling and refreshes it afterward; the provider's conservative periodic update is 30 minutes, while the app refreshes state on entry/pull and after controls.

The Home screen's **Dodaj widget** action calls Android's native pin-widget API on Android 8 or newer. When the active launcher does not support programmatic pinning, the app explains how to add Wariatkowo from the launcher's widget picker. No runtime permission is required or requested. The receiver, widget metadata and launcher-visible label are generated by the same config plugin as the native module.
...
The native dashboard, tasks, shopping list and product history, shop mode, calendar, statistics, profile shell, and Home Assistant controls mirror their web counterparts while using phone-sized vertical sections. Google Calendar connection authorization still opens the authenticated web calendar UI because the existing OAuth flow is deliberately bound to the server's HTTP-only browser session; event browsing, source filters, synchronization, disconnect, and writable event CRUD are native. Widget state is refreshed at action/app/Android periodic-update boundaries rather than through push. Useful future additions remain push reminders, notification actions, biometric session unlock, and a WorkManager-backed explicit sync schedule.
